...
{
    internal class AccessFormatter : SqlFormatter
    {
        protected AccessFormatter(QueryLanguage language) : base(language)
        {
        ...
        {
            return Format(expression, new AccessLanguage());
        }
        ...
        {
            var names = DbNamedValueGatherer.Gather(expression);
        ...
        protected override void WriteColumns(ReadOnlyCollection<DbColumnDeclaration> columns)
        {
            if (columns.Count == 0)
            {
                this.Write("0");
                    ...
                    case "Concat":
                        {
                            ...
                            {
                                args = (args[0] as NewArrayExpression).Expressions;
                            }
